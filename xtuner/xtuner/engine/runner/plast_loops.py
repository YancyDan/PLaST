    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        with autocast(enabled=self.fp16, dtype=torch.float16):
            if is_model_wrapper(self.runner.model):
                ...
                mha = getattr(self.runner.model.module, 'mha', None)
                style_encoder = getattr(self.runner.model.module, 'style_encoder', None)
                ...
                mha = getattr(self.runner.model, 'mha', None)
                style_encoder = getattr(self.runner.model, 'style_encoder', None)
            ...
            if mha is not None:
                if style_encoder is not None:
                    auxi_feature = style_encoder(
                        data['raw_audios'][0].unsqueeze(0).to(audio_tokens.dtype),
                        features_only=True)['x'] # audio_feature from emotion2vec
                else:
                    auxi_feature = data['audio_features'].to(llm_data_dtype)
                ...
                data['audio_features'], _ = mha(audio_tokens, auxi_feature, auxi_feature, auxi_mask)
                # data['audio_features'], _ = mha(auxi_feature, auxi_feature, auxi_feature, auxi_mask)
            else:
                data['audio_features'] = None

            mm_inputs = prepare_inputs_labels_for_plast(
                llm=llm,
