...
def encode_fn(example,
    ...
    src_ids = tokenizer.encode(
        example['src_lang_text'], add_special_tokens=False)
    res = {'input_ids': input_ids, 'labels': labels, 'src_ids': src_ids}

    if use_rep:
        org_sent = example['src_lang_text']
        org_splits = org_sent.split() if ' ' in org_sent else list(org_sent)

        samples = np.random.poisson(lam=1, size=len(org_splits))
        rep_sent = []

        for sent, i in zip(org_splits, samples):
            rep_sent += [sent] * (i + 1)

        rep_sent = ' '.join(rep_sent) if ' ' in org_sent else ''.join(rep_sent)
        res.update({'src_ids_rep': tokenizer.encode(
            rep_sent, add_special_tokens=False)})
