    ...
    for item in raw_data:
        ...
        if mode == 's2t':
            if use_feature:
                conv = {
                    'input':
                    '<audio>\nGiven the paralinguistic information, please translate the {} sentence into {}.\n<audio>'.format(
                        src_lang, tgt_lang),
                    'output':
                    tgt_lang_text
                }
            ...
            if use_feature:
                conv = {
                    'input':
                    '<audio>\nGiven the paralinguistic information, please transcribe the {} sentence into {}.\n<audio>'.format(
                        src_lang, src_lang),
                    'output':
                    src_lang_text
                }
...
def prepare_inputs_labels_for_plast(
        llm: PreTrainedModel,
    ...
    new_labels = []
    prefix_lens = []
    prefix_features = [0] * len(input_ids)
    cur_audio_idx = 0
    cur_feature_idx = 0
    # traverse every instance
    for batch_idx, cur_input_ids in enumerate(input_ids):
        num_audios = (cur_input_ids == LLAST_AUDIO_TOKEN_INDEX).sum()

        # no audio tokens -- TODO: absent audio but increasing idx
        if num_audios == 0:
        ...
        for i in range(num_audios + 1):
            ...
            if i == 0:
                ...
                if audio_features is not None:
                    cur_audio_tokens = audio_features[cur_feature_idx]
                    cur_feature_idx += 1
                ...
                # prefix len
                prefix_lens.append(cur_inputs_embeds_no_audio[i].shape[0])
                # 1st text len + 1st audio len
                prefix_features[batch_idx] += cur_inputs_embeds_no_audio[i].shape[0]
                prefix_features[batch_idx] += cur_audio_tokens.shape[0]
            elif i == 1:
                # audio tokens
                if audio_features is not None:
                    cur_audio_tokens = audio_tokens[cur_audio_idx]
    ...
    for i, (cur_new_embed, cur_new_labels, audio_len, prefix_len) in enumerate(
            zip(new_inputs_embeds, new_labels, audio_lens, prefix_lens)):
        ...
        if cur_len > 0:
            ...
            attention_mask[i, (prefix_len + audio_len)
                           :(prefix_len + max_audio_len)] = False
            if audio_features is not None:
                attention_mask[i, (prefix_features[i] + audio_len)
                               :(prefix_features[i] + max_audio_len)] = False
...
class PLaSTDataset(Dataset):
    ...
    def __init__(self,
        ...
        if offline_processed_text_folder is not None and os.path.exists(
            ...
            if debug:
                # debug_length = int(len(raw_data) / 5)
                debug_length = 500
    ...
    def get_data_ids(self, tsv_dir, split, with_asr, check_audio_path, cv_dir,
                     audio_folder, en2x_sample_step, x2en, en2x, use_feature):
        ...
        for src in x2en:
            ...
            if split == 'train' and with_asr:
                ...
                raw_data_asr, ids_asr = convert_data(
                    data_path, src_lang=SIM_LANG_DICT[src], mode='asr', use_feature=use_feature)
        ...
        if split == 'train' and with_asr and len(en2x) > 0:
            ...
            raw_data_asr, ids_asr = convert_data(
                data_path, src_lang=SIM_LANG_DICT['en'], mode='asr', use_feature=use_feature)
            raw_data += raw_data_asr
            ids += ids_asr
    ...
    def __getitem__(self, index):
        ...
        if self.use_feature:
            fea_path = os.path.join(self.cv_dir, data_dict['src_lang'],
                                    self.feature_folder,
                                    data_dict['ids'].split('.')[0] + '.npy')
            data_dict['audio_feature'] = torch.from_numpy(np.load(fea_path))
            data_dict['feature_len'] = data_dict['audio_feature'].shape[0]
